...
class SSH(object):
	...
	def __init__(self, attuid, rsa_key_path, host):
		'''initialize variables'''
		self.host = host
		self.attuid = attuid
		self.rsa_key_path = rsa_key_path
		self.base_cmd = f'perl /opt/{attuid}/qbot.pl '
		self.cmd = '' 
		self.q_chat = ''
		self.client_ssh = ''
		self.APEX = ''
	...
	def open_ssh_connection(self):
		...
		self.client_ssh.connect( hostname=self.host, username=self.attuid, pkey=rsa_key )
	...
	def create_command(self, message):
		'''create a command to send to qbot'''
		self.cmd = f'{self.base_cmd} {self.q_chat} "{message}"'

	def send_command(self):
		'''send a command to qbot and returns std in/out/err'''
