...
export class SettingsContainerComponent implements OnInit, OnDestroy {
	...
	constructor(private store: Store<{}>, private formBuilder: FormBuilder, private notificationsService: NotificationService, private settingsService: SettingsService, private router: Router) {
		this.settings$ = store.pipe(select(selectSettings)).subscribe((settings: SettingsState) => {
			...
			// once we've retriueved the ssaved settings we know the data
			// is in the store so redirect now for profile guard
			if (this.redirectUrl) {
				this.redirectUrl = '';
	...
	get submitText() {
		const profileHasData = !!(
			...
			this.settings.port ||
			this.settings.devServer ||
			this.settings.emberUrl ||
			this.settings.teamUrl ||
			this.settings.tempUrl
		);
	...
	submit(): void {
		...
		// if given redirect url then save locally for
		// when the store is upodated and erase service url
		if (this.settingsService.redirectUrl) {
			this.redirectUrl = this.settingsService.redirectUrl;
			this.settingsService.redirectUrl = '';
		}
	}
	 ...
	 * @param {AbstractionControl} control the username control object to test
	 ...
	 * @param {AbstractionControl} control the port control object to test
